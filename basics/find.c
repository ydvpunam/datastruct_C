...
void print_list(struct student_t* temp)
{
	...
	while (temp != NULL)
	{
		printf("%d  %d  %c\t", temp->num, temp->rno, temp->name); // Modified by Swastik
		temp = temp->next;
	}
	printf("%\n");
	return;
...
void add_node(struct student_t* temp)
{
	if(temp != NULL)
	{
		...
		temp->next->name = (temp->name + 1); // Added by Swastik
	}
	return;
}
...
int main()
{
	...
	add_node(head);
	print_list(head);
	//find_rno(head); // Commented by Swastik
	struct student_t* found = find_rno(head, 4); // Added by Swastik
	if (found != NULL)
